...
// A11y: keyboard shortcuts are documented in Help and declared in HTML via `aria-keyshortcuts`.
...
function renderSchemes() {
  ...
  SCHEMES.forEach((s, i) => {
    ...
    const shortcutHint = `Shortcut: ${i + 1}`;
    btn.title = locked
      ? `Unlock more scheme slots by ending seasons. · ${shortcutHint}`
      : needsHeat
        ? `Unlock Heat to use Cool Whiskers. · ${shortcutHint}`
        : shortcutHint;
...
window.addEventListener("keydown", (e) => {
  ...
  if (e.key === "j" || e.key === "J") {
    // Jump to Schemes panel (when unlocked).
    if (state.unlocked?.schemes && els.schemes) {
      e.preventDefault();
      const reduceMotion = !!window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches;
      try { els.schemes.scrollIntoView({ block: "start", behavior: reduceMotion ? "auto" : "smooth" }); } catch {}
      setTimeout(() => {
        try {
          const focusable = els.schemes.querySelector("button, select, input, textarea, [tabindex]:not([tabindex='-1'])");
          focusable?.focus?.({ preventScroll: true });
        } catch {}
      }, 0);
    }
